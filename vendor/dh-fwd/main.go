...
func main() {
	...
	flag.BoolVar(&autoYes, "yes", false, "skip confirmation prompts")
	flag.Usage = usage
	...
	// SENTINEL vendor patch — headless configuration from the environment.
	//
	// SENTINEL runs dh-fwd as a spawned helper and must NEVER expose device
	// credentials on the command line (argv is world-readable via /proc and
	// the process list). Credentials, the application profile and the relay
	// switch are therefore taken from the environment when not given as
	// flags. Passing creds via env also implies a non-interactive context,
	// so we skip the confirmation prompt (no TTY is attached).
	if username == "" {
		if v := os.Getenv("DAHUA_P2P_USERNAME"); v != "" {
			username = v
		}
	}
	if password == "" {
		if v := os.Getenv("DAHUA_P2P_PASSWORD"); v != "" {
			password = v
		}
	}
	...
	switch strings.ToLower(strings.TrimSpace(os.Getenv("DAHUA_P2P_RELAY"))) {
	case "1", "true", "yes":
		tcpRelayMode = true
	}
	// Any credentials sourced from the environment mark a headless run:
	// there is no operator to answer a y/n prompt.
	if os.Getenv("DAHUA_P2P_USERNAME") != "" || os.Getenv("DAHUA_P2P_PASSWORD") != "" {
		autoYes = true
	}
...
General:
  ...
  --pool, --pools <n>             pre-bound realms per forwarded port
                                  ...
                                  an explicit value wins over the
                                  automatic pool disable on 2024+
                                  devices / dmss profile)
...
func runMulti(serial string, prof *appProfile, specs []PortSpec, threads int, dtype int, username, password, randsalt string, debug, logRetries bool, tcpRelay bool, poolSize int, poolExplicit bool) {
	...
	for _, g := range distribute(specs, threads) {
		...
		t := newTunnel(serial, prof, dtype, username, password, randsalt, debug, logRetries, tcpRelay, poolSize, poolExplicit, g, reg)
		t.progress = cp
